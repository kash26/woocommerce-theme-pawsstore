...
function pawsgang_scripts() {
	...
	wp_enqueue_style( 'bootstrap-icons', get_template_directory_uri() . '/css/icons-1.5.0/font/bootstrap-icons.css', array(), _S_VERSION );

	wp_enqueue_style( 'pawsgang-main', get_template_directory_uri(  ) . '/dist/main.css' );
	wp_enqueue_style( 'pawsgang-main', '/dist/main.css' );

	// wp_enqueue_style( 'bootstrap-icon', 'https://cdn.jsdelivr.net/npm/bootstrap-icons@1.9.1/font/bootstrap-icons.css', array('jquery') );

	wp_enqueue_script('jquery');

	wp_register_script('pawsgang-popper', get_template_directory_uri() . '/js/popper/src/popper.js', ['jquery'], 1, true);
    wp_enqueue_script('pawsgang-popper');

	wp_register_script('bootstrap-script', get_template_directory_uri() . '/js/bootstrap.min.js', ['jquery'], 1, true);
...
/**
 * Register Custom Google Fonts - font-family: 'Source Sans Pro', sans-serif;
 * Register Custom Google Fonts - font-family: 'Nunito', sans-serif;;
 */
function enqueue_custom_fonts() {
	wp_register_style( 'source_sans_pro', 'https://fonts.googleapis.com/css2?family=Source+Sans+Pro:ital,wght@0,200;0,400;0,600;0,700;0,900;1,200;1,400;1,600;1,700;1,900&display=swap');
...
/**
 * Enqueue your own woo stylesheet
 */
function wp_enqueue_woocommerce_styles() {
	wp_register_style( 'pawsgang', get_template_directory_uri(  ) . '/css/woocommerce/woocommerce.css' );

	if ( class_exists( 'woocommerce' ) ) {
		wp_enqueue_style( 'pawsgang' );
	}
}
add_filter( 'wp_enqueue_scripts', 'wp_enqueue_woocommerce_styles' );